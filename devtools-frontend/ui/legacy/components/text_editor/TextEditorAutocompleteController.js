...
export class TextEditorAutocompleteController {
    ...
    constructor(textEditor, codeMirror, config) {
        ...
        this.tooltipGlassPane.setSizeBehavior("MeasureContent" /* MeasureContent */);
    ...
    initializeIfNeeded() {
        ...
        UI.ARIAUtils.setHasPopup(this.textEditor.element, "listbox" /* ListBox */);
    }
    dispose() {
        // @ts-ignore CodeMirror types are wrong.
        ...
        UI.ARIAUtils.setHasPopup(this.textEditor.element, "false" /* False */);
    }
    beforeChange(codeMirror, changeObject) {
    ...
    addWordsFromText(text) {
        TextUtils.TextUtils.Utils.textToWords(text, this.config.isWordChar, addWord.bind(this));
    ...
    removeWordsFromText(text) {
        TextUtils.TextUtils.Utils.textToWords(text, this.config.isWordChar, word => this.dictionary && this.dictionary.removeWord(word));
    }
    substituteRange(lineNumber, columnNumber) {
        let range = this.config.substituteRangeCallback ? this.config.substituteRangeCallback(lineNumber, columnNumber) : null;
    ...
    wordsWithQuery(queryRange, substituteRange, force) {
        const external = this.config.suggestionsCallback ? this.config.suggestionsCallback(queryRange, substituteRange, force) : null;
        if (external) {
            return external;
        }
    ...
    validateSelectionsContexts(mainSelection) {
        // @ts-ignore CodeMirror types are wrong.
    ...
    setHint(hint) {
        ...
        if (!this.hintMarker) {
            this.hintMarker = this.textEditor.addBookmark(cursor.line, cursor.ch, this.hintElement, TextEditorAutocompleteController.HintBookmark, true);
        }
    ...
    onSuggestionsShownForTest(_suggestions) {
    }
    onSuggestionsHiddenForTest() {
    }
    clearAutocomplete() {
        this.tooltipGlassPane.hide();
        this.hideSuggestBox();
    ...
    async updateTooltip() {
        ...
        this.tooltipGlassPane.show(this.textEditor.element.ownerDocument);
    }
    onCursorActivity() {
        this.updateTooltip();
        ...
        // @ts-ignore CodeMirror types are wrong.
        const cursor = this.codeMirror.getCursor();
        let shouldCloseAutocomplete = !(cursor.line === this.queryRange.startLine && this.queryRange.startColumn <= cursor.ch &&
    ...
    onCursorActivityHandledForTest() {
    }
    updateAnchorBox() {
        if (this.queryRange === null) {
            return;
