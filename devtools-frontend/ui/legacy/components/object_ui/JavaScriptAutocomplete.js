...
export class JavaScriptAutocomplete {
    ...
    constructor() {
        ...
        SDK.ConsoleModel.ConsoleModel.instance().addEventListener(SDK.ConsoleModel.Events.CommandEvaluated, this.clearCache, this);
        UI.Context.Context.instance().addFlavorChangeListener(SDK.RuntimeModel.ExecutionContext, this.clearCache, this);
        SDK.TargetManager.TargetManager.instance().addModelListener(SDK.DebuggerModel.DebuggerModel, SDK.DebuggerModel.Events.DebuggerResumed, this.clearCache, this);
        SDK.TargetManager.TargetManager.instance().addModelListener(SDK.DebuggerModel.DebuggerModel, SDK.DebuggerModel.Events.DebuggerPaused, this.clearCache, this);
    }
    static instance() {
    ...
    async completionsForTextInCurrentContext(fullText, query, force) {
        ...
        const [mapCompletions, expressionCompletions] = await Promise.all([this.mapCompletions(trimmedText, query), this.completionsForExpression(trimmedText, query, force)]);
    ...
    async argumentsForFunction(functionObject, receiverObjGetter, parsedFunctionName) {
        ...
        if (description === 'function () { [native code] }') {
            ...
            if (thisProperty && targetProperty && argsProperty && targetProperty.value && thisProperty.value &&
                ...
                const originalSignatures = await this.argumentsForFunction(targetProperty.value, () => Promise.resolve(thisValue));
    ...
    async mapCompletions(text, query) {
    ...
    async completionsForExpression(fullText, query, force) {
        const executionContext = UI.Context.Context.instance().flavor(SDK.RuntimeModel.ExecutionContext);
        if (!executionContext) {
            return [];
        }
    ...
    receivedPropertyNames(propertyGroups, dotNotation, bracketNotation, expressionString, query) {
        if (!propertyGroups) {
            return [];
        }
    ...
    completionsForQuery(dotNotation, bracketNotation, expressionString, query, propertyGroups) {
...
export class JavaScriptAutocompleteConfig {
    ...
    substituteRange(lineNumber, columnNumber) {
    ...
    async suggestionsCallback(queryRange, substituteRange, force) {
        const query = this.editor.text(queryRange);
        const before = this.editor.text(new TextUtils.TextRange.TextRange(0, 0, queryRange.startLine, queryRange.startColumn));
    ...
    async tooltipCallback(lineNumber, columnNumber) {
