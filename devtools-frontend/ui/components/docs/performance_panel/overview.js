...
async function renderMiniMap(containerSelector, options) {
    ...
    const mainThread = Trace.Handlers.Threads.threadsInRenderer(parsedTrace.Renderer, parsedTrace.AuctionWorklets)
        .find(t => t.type === "MAIN_THREAD" /* Trace.Handlers.Threads.ThreadType.MAIN_THREAD */);
    if (!mainThread) {
    ...
    const zoomedWindow = Trace.Extras.MainThreadActivity.calculateWindow(parsedTrace.Meta.traceBounds, mainThread.entries);
